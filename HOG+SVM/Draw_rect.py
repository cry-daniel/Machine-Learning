...
def detectMultiScale(img, clf, thres, winStride, scale):
    ...
    #plt.show()
    scaledImg=img
    while np.size(scaledImg,0) > 128 and np.size(scaledImg,1) > 64:
        scaledImg=cv.resize(scaledImg, (0, 0), fx=1 / scale, fy=1 / scale)
        rate=(np.size(img,0) / np.size(scaledImg,0), np.size(img,1) / np.size(scaledImg,1))
        for i in range(0, np.size(scaledImg,0) - 128, winStride):
            for j in range(0, np.size(scaledImg,1) - 64, winStride):
...
def pipeline(clf, clsThres, iouThres):
    img=utils.read_img(configs.test_route)
...
#print(utils.cal_HOG(configs.test_route))
#print(clf.predict(utils.cal_HOG(configs.test_route)))
